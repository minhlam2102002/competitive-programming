...
vi SumOfFour(vector<pii> &a, int l, int r, int target) {
    for (int i = l; i <= r - 3; i++) {
        auto res = SumOfThree(a, i + 1, r, target - a[i].first);
        if (res.size() > 0)
            return {a[i].second, res[0], res[1], res[2]};
    }
    return {};
}
...
void solve() {
    ...
    sort(all(a));
    auto res = SumOfFour(a, 0, n - 1, target);
    if (res.size() > 0) {
        forEach(e, res)
            cout << e + 1 << " ";
        cout << endl;
